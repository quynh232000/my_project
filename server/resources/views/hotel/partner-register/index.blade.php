...
@php
    $model      = $params['model'];
@endphp
@section('title', trans('Danh sách đối tác đăng ký'))
@section('main')
<script>
                ...
                <form class="form-horizontal" method="GET" enctype="multipart/form-data" id="admin-form-{{$params['prefix']}}-{{$params['controller']}}" name="admin-form-{{$params['prefix']}}-{{$params['controller']}}">
                    <div class="row m-0 pt-2">
                        <div class="form-group col-sm-3 p-2 mb-0">
                            ...
                            <input id="code" name="code" value="{{isset($params['code']) && !empty($params['code']) ? $params['code'] : ''}}" type="text" class="form-control event-enter" placeholder="Nhập code">
                        </div>
                        <div class="form-group col-sm-3 p-2 mb-0">
                            <label for="title" class="font-weight-normal">Tên khách sạn</label>
                            <input id="title" name="title" value="{{isset($params['title']) && !empty($params['title']) ? $params['title'] : ''}}" type="text" class="form-control event-enter" placeholder="Nhập tên khách sạn">
                        </div>
                        <div class="form-group col-sm-3 p-2 mb-0">
                            <label for="full_name" class="font-weight-normal">Họ tên</label>
                            <input id="full_name" name="full_name" value="{{isset($params['full_name']) && !empty($params['full_name']) ? $params['full_name'] : ''}}" type="text" class="form-control event-enter" placeholder="Nhập họ tên">
                        </div>
                        <div class="form-group col-sm-3 p-2 mb-0">
                            <label for="email" class="font-weight-normal">Email</label>
                            <input id="email" name="email" value="{{isset($params['email']) && !empty($params['email']) ? $params['email'] : ''}}" type="text" class="form-control event-enter" placeholder="Nhập email">
                        </div>
                        <div class="form-group col-sm-3 p-2 mb-0">
                            <label for="phone" class="font-weight-normal">Số điện thoại</label>
                            <input id="phone" name="phone" value="{{isset($params['phone']) && !empty($params['phone']) ? $params['phone'] : ''}}" type="text" class="form-control event-enter" placeholder="Nhập số điện thoại">
                        </div>

                        <div class="form-group col-sm-3 p-2 mb-0">
                            <label for="phone" class="font-weight-normal">Trạng thái</label>
                            {!! \App\Models\Hotel\PartnerRegisterModel::slbStatus($params['status'] ?? '',['all' => true]) !!}
                        </div>
                        ...
                        <div class="row col-12">
                            <div class="form-group col-sm-3 p-2 mb-0">
                                ...
                                <input id="full_name" name="full_name" value="{{isset($params['full_name']) && !empty($params['full_name']) ? $params['full_name'] : ''}}" type="text" class="form-control event-enter" placeholder="Nhập tên nhân viên xử lý">
                            </div>
                            <div class="form-group col-sm-3 p-2 mb-0">
                                <label for="created_at" class="font-weight-normal">Ngày tạo</label>
                                <input id="created_at" name="created_at" value="{{isset($params['created_at']) && !empty($params['created_at']) ? $params['created_at'] : ''}}" type="text" class="form-control event-enter" placeholder="">
                            </div>
                            <div class="form-group col-sm-3 p-2 mb-0">
...
$(document).ready(function(){
    ...
    dateFromTo('#date_change', null, null, 'YYYY/MM/DD')
