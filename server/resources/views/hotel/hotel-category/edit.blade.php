...
<style>
    ...
    <form class="form-horizontal app-container" id="admin-{{ $params['prefix'] }}-form" name="admin-{{ $params['prefix'] }}-form"
        ...
        action="{{ route($params['prefix'] . '.' . $params['controller'] . '.update', ['hotel_category' => $params['item']['id']]) }}">
                        ...
                        <div class="row m-0  pt-2">
                           ...
                           <div class="form-group col-md-6 p-2 mb-0">
                                <label class="col-form-label text-right" for="type">Loại</label>
                                    {!! \App\Models\Hotel\HotelCategoryModel::selectType($params['item']['type'] ?? null) !!}
                                <div class="input-error"></div>
                            </div>

                            <div class="form-group col-6 p-2 mb-0">
                                <label class="col-form-label text-right" for="priority">Thứ tự</label>
                                <input type="text" class="form-control" id="priority" name="priority" placeholder="9999" value="{{$params['item']['priority']}}">
                                <div class="input-error"></div>
                            </div>
                            <div class="form-group col-6 p-2 mb-0">
    ...
    <script>
        $(document).ready(function() {
            $('#admin-{{ $params['prefix'] }}-form').submit(function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route($params['prefix'] . '.' . $params['controller'] . '.update', ['hotel_category' => $params['item']['id']]) }}",
                    data: formData,
                    headers: {
