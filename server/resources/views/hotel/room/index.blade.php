...
@php
    $model      = $params['model'];
@endphp
@section('title', trans('Danh sách loại phòng'))
@section('main')
                ...
                <form class="form-horizontal" method="GET" enctype="multipart/form-data" id="admin-form-{{$params['prefix']}}-{{$params['controller']}}" name="admin-form-{{$params['prefix']}}-{{$params['controller']}}">
                    <div class="row m-0 pt-2">
                        ...
                        <div class="form-group col-3 p-2 mb-0">
                            ...
                            <input id="name" name="name" value="{{isset($params['name']) && !empty($params['name']) ? $params['name'] : ''}}" type="text" class="form-control event-enter" placeholder="Nhập tên dịch vụ">
                        </div>
                        <div class="form-group col-3 p-2 mb-0">
                            <label for="base_price" class="font-weight-normal">Giá loại phòng</label>
                            <input id="base_price" name="base_price" value="{{isset($params['base_price']) && !empty($params['base_price']) ? $params['base_price'] : ''}}" type="text" class="form-control event-enter number_format" placeholder="Nhập giá loại phòng">
                        </div>
                        <div class="form-group col-3 p-2 mb-0">
                            <label for="capacity" class="font-weight-normal">Sức chứa</label>
                            <input id="capacity" name="capacity" value="{{isset($params['capacity']) && !empty($params['capacity']) ? $params['capacity'] : ''}}" type="text" class="form-control event-enter" placeholder="Nhập sức chứa">
                        </div>
                        <div class="form-group col-3 p-2 mb-0">
