...
@section('main')
    ...
    <form class="form-horizontal app-container" id="admin-{{ $params['prefix'] }}-form" name="admin-{{ $params['prefix'] }}-form"
        ...
        action="{{ route($params['prefix'] . '.' . $params['controller'] . '.update', ['accommodation' => $params['item']['id']]) }}">
                    ...
                    <div class="card-body">
                        <div class="row m-0">
                            <div class="form-group col-6 p-2 mb-0">
                                <label class="col-form-label text-right" for="name">Tên loại cư trú<span
                            ...
                            <div class="form-group col-6 p-2 mb-0">
                                ...
                                {!! \App\Models\Hotel\AccommodationModel::slbStatus($params['item']['status']) !!}
                                <div class="input-error"></div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            $('#admin-{{ $params['prefix'] }}-form').submit(function(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route($params['prefix'] . '.' . $params['controller'] . '.update', ['accommodation' => $params['item']['id']]) }}",
                    data: formData,
                    headers: {
