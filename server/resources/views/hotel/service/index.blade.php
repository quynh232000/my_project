...
@php
    ...
    $items      = $model['items'];
@endphp
@section('title', trans('Danh sách dịch vụ'))
@section('main')
...
<div class="row app-container">
                ...
                <form class="form-horizontal" method="GET" enctype="multipart/form-data" id="admin-form-{{$params['prefix']}}-{{$params['controller']}}" name="admin-form-{{$params['prefix']}}-{{$params['controller']}}">
                    <div class="row m-0 pt-2">
                        ...
                        <div class="form-group col-3 p-2 mb-0">
                            <label for="name" class="font-weight-normal">Dịch vụ & tiện ích</label>
                            <input id="name" name="name" value="{{isset($params['name']) && !empty($params['name']) ? $params['name'] : ''}}" type="text" class="form-control event-enter" placeholder="Nhập dịch vụ & tiện ích">
                        </div>
                        <div class="form-group col-3 p-2 mb-0">
                            <label for="name" class="font-weight-normal">Loại tiện ích & dịch vụ</label>
                            <select name="type" id="type" class="form-control select2 select2-blue"
                        ...
                        <div class="form-group col-3 p-2 mb-0">
                            ...
                            {!!\App\Models\Hotel\ServiceModel::slbStatus(@$params['status'],['action' => @$params['action']])!!}
                        </div>
                        <div class="form-group col-3 p-2 mb-0">
                        </div>
                        <div class="form-group col-3 p-2 mb-0">
 ...
 <div class="col-12 text-right d-flex justify-content-end mb-2">
                ...
                <div class="basic-data-table">
                    {!!\App\Models\Hotel\ServiceModel::createTreeTable($params,$items->toTree())!!}
                </div>
                @if (count($items) > 0)
                {!! \App\Models\Hotel\ServiceModel::paginateBackend($items,$params) !!}
                 @endif
