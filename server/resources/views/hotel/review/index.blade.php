...
@php
    $model      = $params['model'];
@endphp
@section('title', trans('Danh sách đánh giá'))
@section('main')
                ...
                <form class="form-horizontal" method="GET" enctype="multipart/form-data" id="admin-form-{{$params['prefix']}}-{{$params['controller']}}" name="admin-form-{{$params['prefix']}}-{{$params['controller']}}">
                    <div class="row m-0 pt-2">
                        <div class="form-group col-3 p-2 mb-0">
                            ...
                            <select name="hotel_id" id="hotel_id" class="form-control select2 select2-blue"
                        ...
                        <div class="form-group col-3 p-2 mb-0">
                            <label for="type" class="font-weight-normal">Loại đánh giá</label>
                            <select name="type" id="type" class="form-control select2 select2-blue" data-dropdown-css-class="select2-blue" style="width: 100%;">
                                <option value="">-- Chọn loại đánh giá --</option>
                                @foreach ($params['reviewType'] as $type)
                                <option value="{{ $type }}" {{isset($params['type']) && $params['type'] == $type ? 'selected' : ''}}>{{ $type }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-3 p-2 mb-0">
                            <label for="status" class="font-weight-normal">{{ trans('field.status_label') }}</label>
                            {!!\App\Models\Hotel\ReviewModel::slbStatus(@$params['status'],['action' => @$params['action']])!!}
                        </div>
