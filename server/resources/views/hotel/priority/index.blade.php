...
@php
    $model      = $params['model'];
@endphp
@section('title', trans('Danh sách khách sạn ưu tiên'))
@section('main')
                    ...
                    <div class="row m-0 pt-2">
                        <div class="col-12 row">
                            <div class="form-group col-3 p-2 mb-0">
                                <label class="font-weight-normal" for="status">Khách sạn</label>
                                {!! \App\Models\Hotel\PriorityModel::selectData($params['hotel_id'] ?? '','hotel_id') !!}
                                <div class="input-error"></div>
                            </div>
                             <div class="form-group col-3 p-2 mb-0">
                                <label for="name" class="font-weight-normal">Danh mục</label>
                            ...
                            <div class="form-group col-3 p-2 mb-0">
                                ...
                                <input id="address" name="address" value="{{$params['address'] ?? ''}}" type="text" class="form-control event-enter" placeholder="Nhập địa chỉ">
                            </div>
                            <div class="form-group col-3 p-2 mb-0">
                                <label class="font-weight-normal" for="status">Trang thái</label>
                                {!! \App\Models\Hotel\ChainModel::slbStatus(isset($params['status']) && !empty($params['status']) ? $params['status'] : '') !!}
                                <div class="input-error"></div>
                            </div>
                        </div>
