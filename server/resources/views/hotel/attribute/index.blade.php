...
@php
    $model = $params['model'];
@endphp
@section('title', trans('Quản lý thông tin chung'))
                        ...
                        <div class="row m-0 pt-2">
                            <div class="form-group col-3 p-2 mb-0">
                                ...
                                <input id="name" name="name"
                                    ...
                                    type="text" class="form-control event-enter" placeholder="Nhập tên danh mục">
                            </div>
                            <div class="form-group col-3 p-2 mb-0">
                                <label for="domain" class="font-weight-normal">Loại</label>
                                <select class="form-control form-select" data-control="select2" id="type" name="type">
                                    <option value="">--- Chọn ---</option>
                                    @foreach ($params['type'] as $type)
                                        <option value="{{ $type['slug'] }}"
                                            {{ ($params['model']['type'] ?? '') == $type['slug'] ? 'selected' : '' }}>
                                            {{ $type['name'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group col-3 p-2 mb-0">
                                <label for="status"
                                    class="font-weight-normal">{{ trans('field.status_label') }}</label>
                                {!! \App\Models\Hotel\AttributeModel::slbStatus(@$params['status'], ['action' => @$params['action']]) !!}
                            </div>
            ...
            <div class="card-body pt-0">
                <div>
                    ...
                    <div class="basic-data-table">
                        {!! \App\Models\Hotel\AttributeModel::createTreeTable($params, $params['model']['items']->toTree()) !!}
                    </div>
                    @if (count($params['model']['items']) > 0)
                        {!! \App\Models\Hotel\AttributeModel::paginateBackend($params['model']['items'], $params) !!}
                    @endif
