...
@php
    $model      = $params['model'];
@endphp
@section('title', trans('Danh sách bài viết'))
@section('main')

<div class="modals" id="global_modal">
    </div>
    ...
    <div class="col-12 p-2">
        <div class="card card-default">
            <div class="card-body p-2">
                <form class="form-horizontal" method="POST" enctype="multipart/form-data" id="admin-form-{{$params['prefix']}}-{{$params['controller']}}" name="admin-form-{{$params['prefix']}}-{{$params['controller']}}">
                    <div class="row m-0 pt-2">
                        <div class="form-group col-3 p-2 mb-0">
                            <label for="name" class="font-weight-normal">Tên</label>
                            <input id="name" name="name" value="{{isset($params['name']) && !empty($params['name']) ? $params['name'] : ''}}" type="text" class="form-control event-enter" placeholder="Nhập tên ">
                        </div>
                        <div class="form-group col-3 p-2 mb-0">
                            <label for="status" class="font-weight-normal">{{ trans('field.status_label') }}</label>
                            {!!\App\Models\Hotel\PostModel::slbStatus(@$params['status'],['action' => @$params['action']])!!}
                        </div>
                        <div class="form-group col-3 p-2 mb-0">
                        </div>
                        <div class="form-group col-3 p-2 mb-0">
