...
class HotelModel extends AdminModel
{
    ...
    public $crudNotAccepted = [
        ...
        'hotel_customer_ids',
        'role',
        'country_id','city_id','ward_id','district_id', 'longitude', 'latitude', 'address',
        'category_ids',
        'image'
    ];
    ...
    public function listItem($params = null, $options = null)
    {
        ...
        if ($options['task']    == "admin-index") {
            ...
            $query->with('customers:id,full_name,email,phone','categories:id,name');
    ...
    public function saveItem($params = null, $options = null)
    {
        if ($options['task'] == 'add-item') {
            ...
            if(isset($options['insert_id'])){
                $this->searchElasticArr($options);
            }
    ...
    public function searchElasticArr(array $options)
    {
        $hotel = self::select('id', 'name', 'slug')
            ->with([
                'location:id,hotel_id,city_name,city_slug,district_name,district_slug,ward_name,ward_slug',
                'categories:id,name,slug'
            ])
            ->find($options['insert_id']);
        $data = [
            'id' => $hotel->id,
            'name' => $hotel->name,
            'slug' => $hotel->slug,
            'categories' => $hotel->categories->map(fn($cat) => [
                'id' => $cat->id,
                'name' => $cat->name,
                'slug' => $cat->slug,
            ])->toArray(),
            'location' => [
                'city_name' => $hotel->location->city_name ?? '',
                'city_slug' => $hotel->location->city_slug ?? '',
                'district_name' => $hotel->location->district_name ?? '',
                'district_slug' => $hotel->location->district_slug ?? '',
                'ward_name' => $hotel->location->ward_name ?? '',
                'ward_slug' => $hotel->location->ward_slug ?? '',
            ],
        ];
        if ($hotel) {
            $elasticService = new ElasticService();
            $elasticService->indexHotel($data);
        }
    }
    ...
    // public function getInfoAddress($params){
    //     $result = [];
    //     switch ($params['type']) {
    //         case 'city_id':
    //             $result = DB::table(TABLE_GENERAL_CITY)->where('country_id',$params['id'])->select('id','name')->get()->toArray();
    //             break;
    //         case 'district_id':
    //             $result = DB::table(TABLE_GENERAL_DISTRICT)->where('city_id',$params['id'])->select('id','name')->get()->toArray();
    //             break;
    //         case 'ward_id':
    //             $result = DB::table(TABLE_GENERAL_WARD)->where('district_id',$params['id'])->select('id','name')->get()->toArray();
    //             break;
    //     }
    //     return $result;
    // }
    // public $dataAddress = [
    //     'city_id'       => TABLE_GENERAL_CITY,
    //     'district_id'   => TABLE_GENERAL_DISTRICT,
    //     'ward_id'       => TABLE_GENERAL_WARD,
    // ];
    // public function searchAddress($params){
    //     return isset($this->dataAddress[$params['type']]) ? DB::table($this->dataAddress[$params['type']])->where('name', 'LIKE', '%' . $params['q'] . '%')->select('id','name')->get()->toArray() : [];
    // }
    ...
    public static function selectCategory($selecteds = [],$is_multi = true){
        $data = HotelCategoryModel::select('id','name')
                                    ->where(['status' => 'active'])
                                    ->get()
                                    ->toArray();
        ...
        foreach ($data as $key => $item) {
            ...
            $selected   = (in_array($item['id'],$selecteds ?? []) ?? false) ? 'selected' : '';
            $opts       .= '<option '.$selected.' value="'.$item['id'].'">'.$item['name'].'</option>';

        }
        $multi = $is_multi ? 'name="category_ids[]" multiple' : 'name="category_id"';
        return  '<select  class="form-control select2" '.$multi.'  data-placeholder="--- Chọn ---">
                    '.($is_multi ?'':'<option value="">--Chọn--</option>').'
                    '.$opts.'
                </select>';
    }
    ...
    public function columnCustomers($params, $field, $val)
    {
        $hotelNames = array_map(function ($customer) {
            return '<a href="'.route('hotel.customer.edit',['customer'=>$customer['id']]).'"
                data-toggle="tooltip" data-placement="top" title="(email)- '.$customer['email'].';
