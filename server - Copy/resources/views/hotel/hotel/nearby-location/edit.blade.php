    ...
    <div class="card">
        ...
        <div class="card-body p-2">
            ...
            <div class="list_location">
                @foreach ($params['item']['near_locations'] as $item)
                    <div class="row m-0 border-bottom parent_location location_item" >
                        <input type="text" hidden name="location_item[]" value="{{$item['id']}}">
                        <div class="form-group col-12 col-lg-4 p-2 mb-0">
                            <label class="col-form-label text-right" for="">
                                Tên địa điểm
                                <span style="color: red">(*)</span>
                            </label>
                            <input type="text" class="form-control name"  name="location_current[{{$item['id']}}][name]"  placeholder="Nhập tên.." value="{{$item['name']}}">
                            <div class="input-error"></div>
                        </div>
                        <div class="form-group col-12 col-lg-4  p-2 mb-0">
                            <div class="d-flex justify-content-between">
                            ...
                            <input type="text" class="form-control"  name="location_current[{{$item['id']}}][address]" placeholder="42 Phan Bội Châu.." value="{{$item['address']}}">
                            <div class="input-error"></div>
                        </div>
                        <div class="form-group col-12 col-lg-4  p-2 mb-0">
                            <div class="d-flex justify-content-between">
                                <label for="address" class="col-form-label text-right">Khoảng cách</label>
                                <button class="btn btn-danger btn-sm location_delete" style="height: fit-content">
                                    <i class="fa-regular fa-trash-can"></i>
                                </button>
                            </div>
                            <input type="text" class="form-control" readonly disabled  placeholder="" value="{{ \App\Models\Hotel\NearbyLocationModel::formatDistance($item['distance'])}}">
                            <div class="input-error"></div>
                        </div>
                        <div class="form-group col-12 col-lg-4  p-2 mb-0">
                            <label for="" >Kinh độ <span  style="color: red">(*)</span></label>
                            <input type="text" class="form-control longitude"  name="location_current[{{$item['id']}}][longitude]" placeholder="Nhập.." value="{{$item['longitude']}}">
                            <div class="input-error"></div>
                        </div>
                        <div class="form-group col-12 col-lg-4  p-2 mb-0">
                            <label for="" >Vĩ độ <span style="color: red">(*)</span></label>
                            <input type="text" class="form-control latitude"  name="location_current[{{$item['id']}}][latitude]" placeholder="Nhập.." value="{{$item['latitude']}}">
                            <div class="input-error"></div>
                        </div>
                        <div class="form-group col-12 col-lg-4  p-2 mb-0">
                            <label for="" >Hoặc Link Google map</label>
            ...
            <button  class="btn btn-primary float-right btn_add_location my-2">
   ...
   const template_location = $('.template_location').show().clone()
    ...
    $('.btn_add_location').click(function(e){//add location
        ...
        $(htmlApend).find('input[name^="location["]').each(function() {
            ...
            const newName     = currentName.replace(/\[0\]/, `[${locationIndex}]`);
