        ...
        <div class="modal-content">
            <form onsubmit="return adminChangeTicket(this,'{{$params['url']}}')" id="confirm-change-ticket" role="form" method="POST"  action="{{$params['url']}}" enctype="multipart/form-data">
                ...
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="deleteModalLongTitle"><i class="mdi mdi-trash-can-outline mdi-18px"></i>
                        @if ($params['type'] == 'choose')
                            Xác nhận xử lý thông tin
                        @else
                            Trạng thái xử lý thông tin
                        @endif
                    </h5>
                ...
                <div class="modal-body">
                    @if ($params['type'] == 'choose')
                        <h5>Xác nhận đồng ý với việc bạn sẽ xử lý thông tin này!</h5>
                    @else
                        <div class="form-group col-12 p-2 mb-0">
                            <label class="col-form-label text-right" for="status">Trạng thái</label>
                            {!! \App\Models\Hotel\PaymentInfoModel::slbStatus($params['item']['status']) !!}
                            <div class="input-error "></div>
                        </div>
                        <div class="form-group col-12 p-2 mb-0">
                            <label class="col-form-label text-right" for="note">Nhân viên ghi chú</label>
                            <textarea class="form-control" name="note" id="note"  placeholder="Nhập ghi chú">{{$params['item']['note']}}</textarea>
                            <div class="input-error "></div>
                        </div>
                    @endif
                </div>
