@extends('layout.app')
@section('title', 'Thêm mới đánh giá')
@section('main')
<style>
    .remove-location {
        position: absolute;
                        ...
                        <div class="row m-0 pt-2">
                            ...
                            <div class="form-group col-6 p-2 mb-0">
                                ...
                                <input type="number" class="form-control" id="like" min="0" name="like"
                                    placeholder="Nhập lượt thích" value="">
                                <div class="input-error"></div>
                            </div>
                            <div class="form-group col-6 p-2 mb-0">
                                ...
                                <div id="quality-container">
                                    <div class="row quality-row">
                                        <div class="form-group col-6 p-2 mb-0">
                                            ...
                                            <select class="form-control select2" id="quality_0" name="qualities[0][quality]">
                                                <option value="">-- Chọn tiêu chí --</option>
                                                @foreach ($params['reviewQuality'] as $quality)
                                                <option value="{{ $loop->index }}">{{ $quality }}</option>
                                                @endforeach
                                            </select>
                                            <div class="input-error"></div>
                                        </div>
                                        <div class="form-group col-5 p-2 mb-0">
                                            <label class="col-form-label text-right" for="quality_point_0"></label>
                                            <input type="number" class="form-control" id="quality_point_0" name="qualities[0][quality_point]" placeholder="Nhập điểm đánh giá" value="">
                                            {{-- <div class="input-error"></div> --}}
                                        </div>
                                        <div class="form-group col-1 p-2 mb-0 ">
                                            <label class="col-form-label text-right" for="quality_point_0"></label>
                                            <button type="button" class="btn btn-danger remove-location float-right btn-sm" data-index="0">
            ...
            <div class="col-6 col-sm-12 col-md-5">
                <div class="card">
                    ...
                    <div class="form-group col-6 p-2 mb-0">
                        ...
                        {!! \App\Models\Hotel\ReviewModel::slbStatus('inactive') !!}
                        <div class="input-error"></div>
                    </div>
                    <!-- /.card-body -->
                </div>
    ...
    <script src="{{ url('assets/plugins/admin-lte/plugins/select2/js/select2.full.min.js') }}"></script>
...
$(formEl).find('.indicator-label').show()
        ...
        $('#quality_point_0').ionRangeSlider({
            min: 1,
            max: 5,
            from: 0,
            to: 5,
        ...
        $(".add-quality").on("click", function() {
            $(".quality-row").find('#quality_point_0').data("ionRangeSlider").destroy();
            $(".quality-row").find('#quality_0').select2('destroy');
            var newQuality  = $(".quality-row").first().clone();
            newQuality.find("input, select").val('');
            newQuality.find("select, input").each(function() {
            ...
            newQuality.find('#quality_point_' + index).removeClass('is-invalid');
            newQuality.find('select').select2({
                    dropdownCssClass: 'select2-blue'
            });
            $(".quality-row").find('#quality_0').select2();
            $(".quality-row").find('#quality_point_0').ionRangeSlider({
                min: 1,
                max: 5,
                from: 0,
                to: 5,
            ...
            newQuality.find('#quality_point_' + index).ionRangeSlider({
                min: 1,
                max: 5,
                from: 0,
                to: 5,
