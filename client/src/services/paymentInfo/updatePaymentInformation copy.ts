...
export const updatePaymentInformation =
	async (body: Partial<PaymentInformationForm> & { id: number }): Promise<IResponseStatus> => {
		try {
			const hotel_id = getClientSideCookie('hotel_id');
			const { data } = await CallAPI().put(AppEndpoint.PAYMENT_INFO + `/${body.id}`, {
				...body,
				hotel_id,
			});
			if (!data) {
