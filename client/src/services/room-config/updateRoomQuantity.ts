...
interface updateRoomQuantityBody {
	start_date: string;
	end_date: string;
	room_ids: number[];
...
export const updateRoomQuantity = async (
	body: updateRoomQuantityBody
): Promise<IResponseStatus> => {
	try {
		const hotel_id = getClientSideCookie('hotel_id');

		const { data } = await CallAPI().post(`${AppEndpoint.ROOM_QUANTITY}`, {
			hotel_id,
			...body,
		});
		if (!data) {
