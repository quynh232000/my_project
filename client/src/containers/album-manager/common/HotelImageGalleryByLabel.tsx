...
const HotelImageGalleryByLabel = ({ label_id }: { label_id: number }) => {
	...
	useEffect(() => {
		if (albumHotel && albumHotel?.hotel && albumHotel.hotel.length > 0 && label_id > 0) {
			setAlbumImages(
				albumHotel.hotel.filter((item) => item.label?.parent_id === label_id)
			);
		}
	}, [albumHotel, label_id]);
	...
	return albumHotel ? (
		<ImageGallerySection
			...
			imageList={albumImages ? formatRoomImagesForImageUpload(albumImages) : []}
		/>
	) : (
		<SkeletonAlbumImage />
	);
