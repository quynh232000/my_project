...
const schemaUploadFile = (isAlbumSchema: boolean) => {
	const imageUploadSchema = validate.array(
		validate.object({
			url: validate.string(),
	...
	const fileUploadSchema = validate
		.array(validate.custom<File>())
		.superRefine(async (files, ctx) => {
			if (files.length === 0) {
				ctx.addIssue({
					code: 'custom',
					message: 'Vui lòng tải ít nhất 1 ảnh',
				});
				return;
			}
...
export const imageGallerySectionSchema = (isAlbumSchema: boolean) => {
	return validate.object({
		...schemaUploadFile(isAlbumSchema),
