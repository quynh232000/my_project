...
export const StandardPriceSchema = z
	.object({
		...
		rate_type: z
			...
			.trim(),
		room_ids: z.array(z.number()).min(1, 'Phải chọn ít nhất một loại phòng'),
		...
		cancellationPolicy: z
			...
			.refine(
				(data) =>
					...
					(data.type === ECancellationPolicy.custom && !!data.policy_cancel_id),
				{
		...
		date_min: z
			.union([
				z.nan(),
				z
					.number({ message: 'Vui lòng nhập số ngày đặt trước tối thiểu' })
		...
		date_max: z
			.union([
				z.nan(),
				z
					.number({ message: 'Vui lòng nhập số ngày đặt trước tối đa' })
		...
		night_max: z
			...
			.refine((val) => !isNaN(val), { message: 'Vui lòng nhập số đêm tối đa' }),
	...
	.refine(
		...
		{
			message: 'Số ngày đặt trước tối thiểu không thể lớn hơn số ngày tối đa',
			path: ['date_min'],
		}
	)
	.refine(
		(data) =>
			!data.night_min || !data.night_max || data.night_min <= data.night_max,
		{
